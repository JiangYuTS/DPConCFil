...
def Table_Interface_Pix(filamentObj):
    ...
    d_outcat = np.hstack([[index_id, CenL, CenB, Length, Area, Ratio, Angle, Clumps]]).T
    columns = ['ID', 'CenL', 'CenB', 'Length', 'Area', 'LWRatio', 'Angle', 'Clumps']
    units = [None, 'pix', 'pix', 'pix', 'pix', None, 'deg', None]
    dtype = ['int', 'float32', 'float32', 'int', 'int', 'float16', 'float16', 'int']
...
def Table_Interface_WCS(filamentObj):
    ...
    index_id = list(np.arange(1, len(CenL) + 1, 1))
    d_outcat = np.hstack([[index_id, CenL, CenB, Length_Arcmin, Area_Arcmin, Ratio, Angle, Clumps]]).T
    columns = ['ID', 'CenL', 'CenB', 'Length', 'Area', 'LWRatio', 'Angle', 'Clumps']
    units = [None, 'deg', 'deg', 'arcmin', 'arcmin^2', None, 'deg', None]
    dtype = ['int', 'float32', 'float32', 'float16', 'float16', 'float16', 'float16', 'int']
