...
def Build_RC_Dict(com,regions_array,regions_first):
    ...
    for cent in center:
        if regions_array[cent[0],cent[1]] != 0 :
            temp_rc_dict[regions_array[cent[0],cent[1]]].append(k1)
            i_record.append(regions_array[cent[0],cent[1]])
        k1 += 1
    for i in range(1,np.int64(regions_array.max())+1):
        if i in i_record:
            coordinates = regions_first[i-1].coords
            temp_regions_array[(coordinates[:,0],coordinates[:,1])] = k2+1
...
#     clump_coords = Alternative_Np_Where(regions_data,arr_sequence,index+1)
    ...
    y_min = core_y.min()
    y_max = core_y.max()
    length = np.max([x_max-x_min,y_max-y_min])+5
    wish_len = 10
    if length<wish_len:
        length = wish_len+5
    ...
    clump_item[core_x-x_min+start_x,core_y-y_min+start_y] = real_data[core_x,core_y]
    ...
    clump_item_region_box[core_x-start_x_for_dilation,core_y-start_y_for_dilation] = 1
