...
#     vector_ab_x = point_b[0] - point_a[0]
    vector_ab_y = point_b[0] - point_a[0]
    vector_ab_z = point_b[1] - point_a[1]
#     vector_ac_x = point_c[0] - point_a[0]
    vector_ac_y = point_c[0] - point_a[0]
    vector_ac_z = point_c[1] - point_a[1]
...
def Get_Line_Logic(centers,center_id,nearest_centers_id,nearest_center_id,regions_data,AllowItems):
    ...
    line_coords_2D = Get_Line_Coords_2D(point_a,point_b)
    for line_coord_2D in line_coords_2D:
        line_data = regions_data[line_coord_2D[0],line_coord_2D[1]]
#         print('line_data:',line_data)
        nearest_centers_id_T = nearest_centers_id.copy()
        nearest_centers_id_T.append(center_id)
        line_data_intersection = (line_data-1) & nearest_centers_id_T
        if len(line_data_intersection) == 0:
            line_zero_record.append(line_coord_2D)
        else:
...
def Cal_Delta_Angle(center_i,center_j,angle):
    l_delta = center_i[1]-center_j[1]
    b_delta = center_i[0]-center_j[0]    
        ...
        if nearest_center_id_t != center_id and nearest_center_id_t != nearest_center_id:
            ...
            if dist_ab_c < TolDistance:
                ...
                if line_logic_0 and line_logic_1:
                    ...
                    pline_1 = [centers[center_id][0],centers[center_id][1]]
                    pline_2 = [centers[nearest_center_id][0],centers[nearest_center_id][1]]
                    pitem = [centers[nearest_center_id_t][0],centers[nearest_center_id_t][1]]
...
def Get_Related_Ids_RR(regions_data,centers,rr_centers_id,connected_ids_dict,edges,angles,TolAngle,TolDistance,AllowItems):
    related_ids = {}
    line_items = {}
    between_items = {}
    crossing_items = {}
