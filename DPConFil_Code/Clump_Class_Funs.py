...
def Detect_From_Regions(clumpsObj):
    ...
    mask_name,outcat_name,outcat_wcs_name = clumpsObj.mask_name,clumpsObj.outcat_name,clumpsObj.outcat_wcs_name
    origin_data = clumpsObj.origin_data
...
#         np.savez(outcat_name[:-4] + '_FacetClumps_npz', did_FacetClumps=did_table)
        ...
        td_outcat, td_outcat_wcs = FacetClumps.Detect_Files_Funs.Table_Interface(did_table, clumpsObj.data_header, ndim)
...
def Single_Gaussian_Fit(*parameters_init):
    Single_Gauss = lambda x,y: \
                    parameters_init[0]*np.exp(-((x-parameters_init[1])**2*(np.cos(parameters_init[5])**2/(2*parameters_init[3]**2)+\
                                                np.sin(parameters_init[5])**2/(2*parameters_init[4]**2))+\
                    (x-parameters_init[1])*(y-parameters_init[2])*(np.sin(2*parameters_init[5])/(2*parameters_init[4]**2)-\
...
#     clump_coords = Alternative_Np_Where(regions_data,arr_sequence,index+1)
    ...
    clump_item[core_x-x_min+start_x,core_y-y_min+start_y,core_z-z_min+start_z] = real_data[core_x,core_y,core_z]
    center_index = [centers[index][0]-x_min+start_x,centers[index][1]-y_min+start_y,centers[index][2]-z_min+start_z]
    ...
    clump_region_box = regions_data[start_x_for_dilation:start_x_for_dilation+length,\
                                    start_y_for_dilation:start_y_for_dilation+length,start_z_for_dilation:start_z_for_dilation+length]
    clump_item_region_box =  np.zeros_like(clump_region_box)
    clump_item_region_box[core_x-start_x_for_dilation,core_y-start_y_for_dilation,core_z-start_z_for_dilation] = 1
    selem = morphology.ball(1)
    clump_item_region_box_dilated = morphology.dilation(clump_item_region_box, selem=selem)
...
def Get_Data_Ranges_WCS(origin_data,data_wcs):
    ...
    if data_wcs.naxis==4:
        ...
        data_ranges_end = data_wcs.all_pix2world(origin_data_shape[2],origin_data_shape[1],origin_data_shape[0],0,0)
    elif data_wcs.naxis==3:
        data_ranges_start = data_wcs.all_pix2world(0,0,0,0)
        data_ranges_end = data_wcs.all_pix2world(origin_data_shape[2],origin_data_shape[1],origin_data_shape[0],0)
