...
def Pedal_Point(pline_1, pline_2, pitem):
    if pline_1[0] != pline_2[0]:
        # ########## 根据点x1和x2计算线性方程的k, b
        k, b = np.linalg.solve([[pline_1[0], 1], [pline_2[0], 1]], [pline_1[1], pline_2[1]])  # 得到k和b
        # #######原理: 垂直向量数量积为0
        x = np.divide(((pline_2[0] - pline_1[0]) * pitem[0] + (pline_2[1] - pline_1[1]) * pitem[1] - b * (pline_2[1] - pline_1[1])),
                      (pline_2[0] - pline_1[0] + k * (pline_2[1] - pline_1[1])))
        y = k * x + b
    else:  # 点pline_1和pline_2的连线垂直于x轴时
        x = pline_1[0]
        y = pitem[1]
...
def Get_Crossing_Items(regions_data,centers,center_id,nearest_centers_id,nearest_center_id,\
    ...
    items = list(set([center_id]+[nearest_center_id]+crossing_items[min_center_id][0]+crossing_items[min_center_id][1]))
...
#     line_data_dict = {}
#     min_center_id = np.min([center_id, nearest_center_id])
#     crossing_items_i[min_center_id] = []
#     between = 0
#     for nearest_center_id_t in nearest_centers_id:
#         if nearest_center_id_t != center_id and nearest_center_id_t != nearest_center_id:
#             dist_ab_c = Dist_Line_Point(centers[center_id], centers[nearest_center_id], centers[nearest_center_id_t])
...
#                 if line_logic_0 and line_logic_1:
#                     line_data_dict[nearest_center_id_t] = list(set(line_data_0+line_data_1))
#                     if nearest_center_id_t not in crossing_items_i[min_center_id]:          
#                         crossing_items_i[min_center_id].append(nearest_center_id_t)
#     #判断是连线上的目标还是中间的目标
#     nearest_center_id_temps = []
#     if len(line_data_set)>2:
#         remove_flag = 1
#         for nearest_center_id_t in crossing_items_i[min_center_id]:
#             if nearest_center_id_t in line_data_set:
#                 remove_flag = 0
#                 crossing_items[min_center_id][0] += [nearest_center_id_t]
#             else:
#                 crossing_items[min_center_id][1] += [nearest_center_id_t]
#             nearest_center_id_temps.append(nearest_center_id_t)
#     elif len(line_data_set) == 2:
#         remove_flag = 0
#         crossing_items[min_center_id][1] += crossing_items_i[min_center_id]
#     #所有在line_data_set内的都需要满足TolDistance才允许保留
#     if len(line_data_set)-len(crossing_items[min_center_id][0])>2:
#         remove_flag = 1
#         for nearest_center_id_t in nearest_center_id_temps:
...
def Add_Items_To_Related_Center(related_center,crossing_items,new_items_dict,item_flag):
    ...
    for key in keys:
        if len(crossing_items[key][item_flag]) != 0:
            ...
            for item in items_set:
                if item not in related_center[key]:
                    valid_item.append(item)
            if len(valid_item) != 0:
                new_items_dict[key] = list(set(valid_item))
                related_center[key] += valid_item
    return related_center,new_items_dict

def Get_Related_Center_RR(regions_data,centers,rr_centers_id,connected_ids_dict,edges,angles,TolAngle,TolDistance,AllowItems):
    related_center = {}
    line_items = {}
    between_items = {}
    crossing_items = {}
    for center_id in rr_centers_id:
        related_center[center_id] = []
        ...
        if edges[center_id] == 0:# or edges[center_id] == 1
            ...
            for nearest_center_id in nearest_centers_id:
                if edges[nearest_center_id] == 0:
                    ...
                    line_logic,line_data_set = Get_Line_Logic(centers,center_id,nearest_centers_id,nearest_center_id,regions_data,AllowItems)
                    if delta_angle_1 < TolAngle and delta_angle_2 < TolAngle and line_logic and center_id<nearest_center_id:
                        related_center[center_id].append(nearest_center_id)
                        ...
                        crossing_items,remove_flag = Get_Crossing_Items(regions_data,centers,center_id,\
                                nearest_centers_id_ij,nearest_center_id,crossing_items,line_data_set,TolDistance,AllowItems)
                        if remove_flag:
                            related_center[center_id].remove(nearest_center_id)
                        else:
                            ...
                            for line_center_id in line_centers_id:
                                ...
                                if remove_flag:
                                    related_center[center_id].remove(nearest_center_id)
                                    break
    related_center,between_items = Add_Items_To_Related_Center(related_center,crossing_items,between_items,0)
    related_center,line_items = Add_Items_To_Related_Center(related_center,crossing_items,line_items,1)
    return related_center,between_items,line_items

def Update_Related_Center(related_center):
    related_center_1 = {}
    related_center_2 = {}
    keys = np.array(list(related_center.keys()))
    for key in keys:
        related_center_1[key] = []
    key_used = []
    loop_i = 1
    for key_0 in keys:
        for key_1 in keys:
            for key_1_item in related_center[key_1]:
                if (key_0 in related_center[key_1] or key_1 in related_center[key_0] or key_1_item in related_center[key_0]) \
                    and key_1 not in key_used:
                    related_center_1[key_0] += related_center[key_0]
                    related_center_1[key_0] += related_center[key_1]
                    related_center_1[key_0].append(key_1)
                    key_used.append(key_1)
        loop_i += 1
    for key in related_center_1.keys():
        if len(related_center_1[key]) != 0:
            related_center_2[key] = list(set(related_center_1[key]))
#             related_center_2[key].remove(key)    
    return related_center_2
