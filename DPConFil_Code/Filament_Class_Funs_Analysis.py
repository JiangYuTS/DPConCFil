...
#                 weight_ij = origin_data[filament_centers_LBV[i][2],filament_centers_LBV[i][1],filament_centers_LBV[i][0]] + \
    ...
    for i in range(len(degree1_nodes)-1):
        for j in range(i+1, len(degree1_nodes)):
            if nx.has_path(T, degree1_nodes[i], degree1_nodes[j]):
                ...
                for k in range(len(path)-1):
#                     weight_ij = \
#                     origin_data[filament_centers_LBV[path[k]][2],filament_centers_LBV[path[k]][1],filament_centers_LBV[path[k]][0]] + \
#                     origin_data[filament_centers_LBV[path[k+1]][2],filament_centers_LBV[path[k+1]][1],filament_centers_LBV[path[k+1]][0]]
                    line_coords = Get_Line_Coords_3D(filament_centers_LBV[path[k]],filament_centers_LBV[path[k+1]])
                    weight_ij = origin_data[line_coords[:,2],line_coords[:,1],line_coords[:,0]].mean()
                    path_weight += dist_matrix[path[k],path[k+1]]*np.abs(points_V[path[k]]-points_V[path[k+1]])*weight_ij
#                     path_weight += T[path[k]][path[k+1]]['weight']
                paths_and_weights.append((path, path_weight))
    if len(paths_and_weights) != 0:
        max_weight = max([weight for path, weight in paths_and_weights])
...
def Trim_Skeleton_Coords_2D(skeleton_coords_2D,fil_mask):
    id_item = 1
    ...
    for skeleton_index in range(0,len(props)):
        if len(props[skeleton_index].coords)>8 and skeleton_index!=longest_skeleton_index:
            skeleton_coords_2D = np.r_[skeleton_coords_2D,props[skeleton_index].coords]   
    if len(skeleton_coords_2D)>4:  
...
#     G_longest_skeleton,T_longest_skeleton = Graph_Infor_Connected(skeleton_coords)
    ...
    for end_point_id in range(len(end_points)):
        ...
        while True:
            add_coord += direction
            if not (0 <= round(add_coord[0]) < filament_mask_2D.shape[0] and 0 <= round(add_coord[1]) < filament_mask_2D.shape[1]):
                break
...
def Get_Longest_Skeleton_Coords(filament_mask_2D):
    ...
    closing_mask = morphology.closing(filament_mask_2D, selem=selem)
...
def Get_Single_Filament_Skeleton(filament_mask_2D):
    ...
    skeleton_coords_2D = Trim_Skeleton_Coords_2D(skeleton_coords_2D,filament_mask_2D)
...
def Get_Sub_Mask(point,regions_data,related_center_ids,connected_ids_dict,clump_coords_dict):
    if len(regions_data.shape) == 2:
        ...
        fil_sub_mask_profiles = np.zeros_like(regions_data)
        for connected_id in connected_ids_i:
            if connected_id in related_center_ids:
                fil_sub_mask_profiles[clump_coords_dict[connected_id]] = 1
        ...
        for connected_id in connected_ids_i:
            if connected_id in related_center_ids:
                if connected_id in related_center_ids:
                    ...
                    fil_mask_sub_profiles[(coords[1],coords[2])] = 1
    return fil_mask_sub_profiles

def Cal_Dictionary_Cuts(SampInt,CalSub,regions_data,related_center_ids,connected_ids_dict,clump_coords_dict,\
    ...
    if len(skeleton_coords_2D)>3:
        ...
        for point_id in range(len(points)):
            ...
            if np.round(points[point_id][0]) > 0 and np.round(points[point_id][1]) >0 \
                and np.round(points[point_id][0]) < fil_image_shape[1]-1 and np.round(points[point_id][1]) < fil_image_shape[0]-1: 
                if CalSub:
                    fil_mask = Get_Sub_Mask(points[point_id],regions_data,related_center_ids,connected_ids_dict,clump_coords_dict)
...
#     degree1_nodes = [node for node in Tree.nodes if Tree.degree(node) == 1 and contour_data[mask_coords[node][0],mask_coords[node][1]]]
#     max_path = []
#     max_num_nodes = -float('inf')
#     for i in range(len(degree1_nodes)-1):
#         for j in range(i+1, len(degree1_nodes)):
...
def Get_Single_Filament_Skeleton_Weighted(fil_image,fil_mask,clump_numbers):
    fil_image_filtered = ndimage.uniform_filter(fil_image, size=3)
#     fil_image_filtered = ndimage.maximum_filter(fil_image, size=5)
#     selem = morphology.square(5)
#     fil_image_filtered = filters.median(fil_image,selem=selem)
#     fil_image_filtered = filters.gaussian(fil_image,0.6)
#     fil_mask = ndimage.binary_fill_holes(fil_mask)
    ...
    for i,j in zip(mask_coords_in_dm[0],mask_coords_in_dm[1]):
        weight_ij = fil_image_filtered[mask_coords[i][0],mask_coords[i][1]] + fil_image_filtered[mask_coords[j][0],mask_coords[j][1]]
        if weight_ij == 0:
            weight_ij = 0
        else:
    ...
    if clump_numbers < 100:
        max_path,max_edges = Get_Max_Path_Intensity_Weighted(fil_mask,Tree,mask_coords)
    else :
...
#     skeleton_coords_2D,filament_skeleton,longest_skeleton_coords = Get_Single_Filament_Skeleton(fil_mask)
    skeleton_coords_2D = Trim_Skeleton_Coords_2D(skeleton_coords_2D,fil_mask)
    return skeleton_coords_2D

def Cal_Lengh_Width_Ratio(CalSub,regions_data,related_center_ids,connected_ids_dict,clump_coords_dict,\
    ...
    if len(regions)>1:
        ...
        for region in regions:
            ...
            if region.area > max_area:
                ...
                max_region = region
        fil_image[max_region.coords[:,0],max_region.coords[:,1]] = filament_data.sum(0)[max_region.coords[:,0],max_region.coords[:,1]]
    ...
    if SkeletonType == 'Morphology':
        ...
        skeleton_coords_2D = Get_Single_Filament_Skeleton_Weighted(fil_image,fil_mask,clumps_number)
    else:
        print('Please choose the skeleton_type between Morphology and Intensity')
        
    if len(skeleton_coords_2D)>3:
        dictionary_cuts = Cal_Dictionary_Cuts(samp_int,CalSub,regions_data,related_center_ids,connected_ids_dict,clump_coords_dict,\
                                        skeleton_coords_2D,fil_image,fil_mask,dictionary_cuts)
...
def Cal_V_Com_Delta(filament_data,skeleton_coords_2D,data_ranges_lbv):
    filament_shape = filament_data.shape
    velocity_range = np.linspace(data_ranges_lbv[2][0], data_ranges_lbv[2][1],filament_shape[0]+1)[:-1]
    velocity_map_i = np.tensordot(filament_data, velocity_range, axes=((0,), (0,))) / np.sum(filament_data, axis=0)
    velocity_map_i = np.nan_to_num(velocity_map_i)
    v_skeleton_com_i = velocity_map_i[(skeleton_coords_2D[:,0],skeleton_coords_2D[:,1])]
    v_skeleton_com_delta = np.around(v_skeleton_com_i.max()-v_skeleton_com_i.min(),3)
    return velocity_map_i,v_skeleton_com_delta
