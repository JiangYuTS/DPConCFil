...
def Plot_Filament_Item(filamentObj,figsize=(8,6),fontsize=12,spacing=12*u.arcmin,save_path=None):
    ...
    dictionary_cuts_item = filamentObj.dictionary_cuts
    ...
    vmin = np.min(filament_data.sum(0)[np.where(filament_data.sum(0)!=0)])
    vmax = np.nanpercentile(filament_data.sum(0)[np.where(filament_data.sum(0)!=0)], 98.)
    ax0.imshow(filament_data.sum(0),
               origin='lower',
               cmap='gray',
               interpolation='none',
               norm = colors.Normalize(vmin = vmin, vmax =  vmax))
    ax0.contourf(filament_data.sum(0),
                 levels = [0., .01],
                 colors = 'w')
    ...
    xmin_coord = max(filament_item.shape[2]/12, xmin-.1*(xmax-xmin)+filament_item.shape[2]/15)
    ymax_coord = min(filament_data.shape[1]-filament_item.shape[1]/15, ymax+.1*(ymax-ymin)-filament_item.shape[1]/15)
    ax0.text(xmin_coord,ymax_coord,r'Center: [{}°,{}°,{}km/s]'.\
    ...
    ax0.text(xmin_coord,ymax_coord-filament_item_shape[1]/8,r'Ratio={}'.\
...
def Plot_PV_Integrate(filamentObj,figsize=(10,8),fontsize=12,spacing=[0.2,0.2,None],save_path=None):
    ...
    wcs_ticks_v = Get_WCS_Ticks(data_item_ranges_lbv[2],delta_sign=1,delta_interval=delta_interval_v,decimal_digits=0)
