...
def Cal_Mean_Profile(filamentObj,EProfileTime=2,EProfileLen=6,ExtendRange=20):
    ...
    max_range = np.int(np.max([np.abs(np.min(xall_peak_eff)-ExtendRange), np.int(np.max(xall_peak_eff))+ExtendRange]))
    bins = np.linspace(-max_range, max_range, 2*max_range+1)
    axis_coords = bins[:-1] # +.5*np.diff(bins)
    for axis_coords_i in axis_coords:
        yall_bin_i = yall_peak[((xall_peak >= (axis_coords_i-.5*np.diff(bins)[0])) & (xall_peak < (axis_coords_i+.5*np.diff(bins)[0])))]
...
def Cal_Profile_IOU(filamentObj,MeanProfile=True):
    if MeanProfile:
        ...
        for i in range(0,len(dictionary_cuts['distance'])):
            ...
            max_range_i = np.int(np.max([np.abs(np.min(dists_i)-filamentObj.ExtendRange), np.int(np.max(dists_i))+filamentObj.ExtendRange]))
            bins = np.linspace(-max_range_i, max_range_i, 2*max_range_i+1)
            axis_coords = bins[:-1]
            mean_profile_i = np.zeros(2*max_range_i)
            j = 0
            for axis_coords_i in axis_coords:
                y_bin_i = y_peak_i[((x_peak_i >= (axis_coords_i-.5*np.diff(bins)[0])) & (x_peak_i < (axis_coords_i+.5*np.diff(bins)[0])))]
...
def Fit_Profile(filamentObj,FitFunc='Gaussian',FitDist=None,FitMeanProfile=False,BGDist=None,BGDegree=0,BeamWidth=None):
    ...
    if filamentObj.radObj.FitDist is None:
        FitDist = np.int(np.abs(filamentObj.xall_peak_eff).max()+0.5)
    else:
        FitDist = filamentObj.radObj.FitDist
    filamentObj.radObj.fit_profile(fitfunc=FitFunc,fitdist=FitDist,bgdist=BGDist,bgdegree=BGDegree, beamwidth=BeamWidth)
